...
export const edgeFunctionAccessMatrix: EdgeFunctionAccessMatrixEntry[] = [
  ...
  { name: "enrich-wiki-tags", verifyJwt: true, audience: "admin", authCheck: "Authorization header resolved with auth.getUser", roleCheck: "Admin enforced through user_roles lookup", roleEnforcement: "admin enforced via service-role user_roles lookup after auth.getUser", rateLimitPolicy: "No local rate limit documented; admin-only JWT and CORS restrict access.", usesServiceRole: true, cors: "request-aware allowlist", handlesPii: false, publicRationale: "" },
  { name: "extract-lab-image", verifyJwt: true, audience: "admin", authCheck: "Authorization header resolved with auth.getUser", roleCheck: "Admin enforced through user_roles lookup", roleEnforcement: "admin enforced via service-role user_roles lookup after auth.getUser", rateLimitPolicy: "No local rate limit documented; admin-only JWT and CORS restrict access.", usesServiceRole: true, cors: "request-aware allowlist", handlesPii: true, publicRationale: "" },
  { name: "generate-diagnoses", verifyJwt: true, audience: "admin", authCheck: "Authorization header resolved with auth.getUser", roleCheck: "Admin enforced through user_roles lookup", roleEnforcement: "admin enforced via service-role user_roles lookup after auth.getUser", rateLimitPolicy: "No local rate limit documented; admin-only JWT and CORS restrict access.", usesServiceRole: true, cors: "request-aware allowlist", handlesPii: true, publicRationale: "" },
  { name: "generate-icd10", verifyJwt: true, audience: "admin", authCheck: "Authorization header resolved with auth.getUser", roleCheck: "Admin enforced through has_role RPC", roleEnforcement: "admin enforced via has_role RPC after auth.getUser", rateLimitPolicy: "No local rate limit documented; admin-only JWT and CORS restrict access.", usesServiceRole: true, cors: "request-aware allowlist", handlesPii: true, publicRationale: "" },
  { name: "get-patients", verifyJwt: true, audience: "admin", authCheck: "Authorization bearer token resolved with auth.getUser", roleCheck: "Admin enforced through has_role RPC", roleEnforcement: "admin enforced via has_role RPC after auth.getUser", rateLimitPolicy: "No local rate limit documented; admin-only JWT and CORS restrict access.", usesServiceRole: true, cors: "request-aware allowlist", handlesPii: true, publicRationale: "" },
  { name: "get-therapy-sessions", verifyJwt: true, audience: "admin", authCheck: "Authorization bearer token resolved with auth.getUser", roleCheck: "Admin enforced through has_role RPC", roleEnforcement: "admin enforced via has_role RPC after auth.getUser", rateLimitPolicy: "No local rate limit documented; admin-only JWT and CORS restrict access.", usesServiceRole: true, cors: "request-aware allowlist", handlesPii: true, publicRationale: "" },
  { name: "list-therapy-pseudonyms", verifyJwt: true, audience: "admin", authCheck: "Authorization bearer token resolved with auth.getUser", roleCheck: "Admin enforced through has_role RPC", roleEnforcement: "admin enforced via has_role RPC after auth.getUser", rateLimitPolicy: "No local rate limit documented; admin-only JWT and CORS restrict access.", usesServiceRole: true, cors: "request-aware allowlist", handlesPii: true, publicRationale: "" },
  { name: "notify-existing-patient", verifyJwt: true, audience: "authenticated", authCheck: "Supabase platform JWT via verify_jwt", roleCheck: "Authenticated post-registration notification context; no admin role", roleEnforcement: "verify_jwt platform JWT only; no admin role or user_roles lookup", rateLimitPolicy: "No local rate limit documented; authenticated notification context plus CORS restricts access.", usesServiceRole: false, cors: "request-aware allowlist", handlesPii: true, publicRationale: "" },
  ...
  { name: "resend-submission", verifyJwt: true, audience: "admin", authCheck: "Authorization header resolved with auth.getUser", roleCheck: "Admin enforced through has_role RPC", roleEnforcement: "admin enforced via has_role RPC after auth.getUser", rateLimitPolicy: "No local rate limit documented; admin-only JWT and CORS restrict access.", usesServiceRole: true, cors: "request-aware allowlist", handlesPii: true, publicRationale: "" },
  { name: "send-icd10-report", verifyJwt: true, audience: "admin", authCheck: "Authorization bearer token resolved with auth.getUser", roleCheck: "Admin enforced through user_roles lookup", roleEnforcement: "admin enforced via service-role user_roles lookup after auth.getUser", rateLimitPolicy: "No local rate limit documented; admin-only JWT and CORS restrict access.", usesServiceRole: true, cors: "request-aware allowlist", handlesPii: true, publicRationale: "" },
  { name: "send-verification-email", verifyJwt: false, audience: "public-pre-session", authCheck: "Legacy application-level verification email flow", roleCheck: "None before session", roleEnforcement: "public legacy pre-session flow; no role before session", rateLimitPolicy: "Legacy no local rate limit documented; retained only as existing pre-session compatibility path.", usesServiceRole: false, cors: "public/pre-session cors", handlesPii: true, publicRationale: "Legacy pre-session verification flow remains intentionally reachable until replacement/legal review is complete." },
  ...
  { name: "therapy-recommend", verifyJwt: true, audience: "admin", authCheck: "Authorization header resolved with auth.getUser", roleCheck: "Admin enforced through has_role RPC", roleEnforcement: "admin enforced via has_role RPC after auth.getUser", rateLimitPolicy: "No local pre-call rate limit documented; upstream/provider 429 is passed through when returned.", usesServiceRole: true, cors: "request-aware allowlist", handlesPii: true, publicRationale: "" },
