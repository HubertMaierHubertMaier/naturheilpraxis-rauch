      ...
      <div
        ...
        onDrop={event => {
          ...
          if ([...event.dataTransfer.items].some(item => typeof item.webkitGetAsEntry === "function" && item.webkitGetAsEntry()?.isDirectory)) {
            setNotice(mode === "single" ? "Für einen Ordner zuerst die Sammeleingabe wählen und dort „Ordner mit PDFs auswählen“ verwenden." : "Für einen Ordner bitte die Schaltfläche „Ordner mit PDFs auswählen“ verwenden."); return;
          }
          acceptFiles(Array.from(event.dataTransfer.files));
        }}
      >
        <FileUp className="mx-auto mb-2 h-7 w-7 text-primary" />
        <p className="font-semibold">{mode === "single" ? "Eine PDF dieses Patientenfalls hier hineinziehen" : "Mehrere PDFs dieses Patientenfalls hier hineinziehen"}</p>
