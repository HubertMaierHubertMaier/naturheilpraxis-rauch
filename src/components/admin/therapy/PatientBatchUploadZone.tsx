  ...
  return (
    <div className="space-y-3">
      <div role="radiogroup" aria-label="Dokumente einzeln oder gesammelt übernehmen" className="grid gap-3 sm:grid-cols-2">
        ...
        ] as const).map(([value, title, description]) => <button key={value} type="button" role="radio" aria-checked={mode === value} disabled={selectionLocked} onClick={() => onModeChange?.(value)} className={`rounded-xl border-2 p-4 text-left transition-all ${mode === value ? "border-emerald-700 bg-emerald-100 text-emerald-950 shadow-md ring-2 ring-emerald-500/70 dark:border-emerald-300 dark:bg-emerald-950/50 dark:text-emerald-50" : "border-border bg-background hover:border-primary/40"}`}><span className="block text-base font-semibold">{title}</span><span className={`mt-1 block text-sm ${mode === value ? "text-emerald-800 dark:text-emerald-100" : "text-muted-foreground"}`}>{description}</span></button>)}
      </div>
