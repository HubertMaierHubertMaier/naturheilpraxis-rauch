  ...
  return (
    <div className="space-y-3">
      <div role="radiogroup" aria-label="Dokumente einzeln oder gesammelt übernehmen" className="grid gap-3 sm:grid-cols-2">
        ...
        ] as const).map(([value, title, description]) => <button key={value} type="button" role="radio" aria-checked={mode === value} disabled={selectionLocked} onClick={() => onModeChange?.(value)} className={`rounded-xl border-2 p-4 text-left transition-all ${mode === value ? "border-blue-800 bg-blue-700 text-white shadow-lg ring-2 ring-blue-500 ring-offset-2 dark:border-blue-200 dark:bg-blue-600" : "border-border bg-background hover:border-primary/40"}`}><span className="block text-base font-semibold">{title}</span><span className={`mt-1 block text-sm ${mode === value ? "text-blue-50" : "text-muted-foreground"}`}>{description}</span></button>)}
      </div>
