  ...
  const downloadAreaBackup = async (area: BackupArea) => {
    ...
    try {
      ...
      const wikiBridgeRestoreLines = area.id === "wiki" ? [
        ...
        "4. Vorhandene Wiki-Daten einschliesslich der durch Migrationen angelegten Seeds mit `DELETE` in umgekehrter Fremdschluesselreihenfolge leeren: `kb_search_documents` vor `kb_release_items`, `kb_release_items` vor `kb_releases`, `kb_safety_rule_conditions` vor `kb_safety_rules` und beide Regelarten vor ihren Assertions. Bestehende `therapy_input_facts` bleiben unangetastet. Ihr `kb_entity_id`-Fremdschluessel ist `NO ACTION DEFERRABLE` und darf erst nach dem Reimport derselben Entitaets-UUIDs geprueft werden. `kb_entities` deshalb weder per `TRUNCATE` noch mit `CASCADE` leeren.",
