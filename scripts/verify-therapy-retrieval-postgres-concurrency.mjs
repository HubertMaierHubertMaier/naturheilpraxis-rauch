...
assert.ok(
  ...
  "The concurrency probe may use only the local ephemeral PostgreSQL service",
);
...
const callerCount = 4;
const applicationName = "therapy-retrieval-step7f-concurrency";
const advisoryBarrierKey = 7340031;
...
async function waitForBlockedCallers(client) {
  ...
  throw new Error("Concurrent callers did not reach the PostgreSQL barrier");
}
...
try {
  ...
  for (const [key, value] of Object.entries(fixture)) {
    assert.ok(value, `Synthetic concurrency fixture is missing ${key}`);
  }
  await fixtureReader.end();
  fixtureReader = undefined;

  const emptyInventory = await inspector.query(`
    SELECT count(*)::integer AS count,
           public.therapy_retrieval_v2_invalid_audit_run_count_v1()::integer
             AS invalid
    FROM public.therapy_retrieval_audit_runs
  `);
  assert.deepEqual(emptyInventory.rows[0], { count: 0, invalid: 0 });
  ...
  const calls = callers.map(async (client) => {
    ...
    return { value: result.rows[0].value, durationMs: Date.now() - startedAt };
  });
  let barrierWaiters;
  let results;
  try {
    barrierWaiters = await waitForBlockedCallers(inspector);
  ...
  assert.equal(
    results.filter(({ value }) => value.status === "RETRIEVAL_AUDIT_PERSISTED_INACTIVE").length,
    1,
  );
  assert.equal(
    results.filter(({ value }) => value.status === "RETRIEVAL_AUDIT_ALREADY_PERSISTED_INACTIVE").length,
    callerCount - 1,
  );
  assert.equal(new Set(results.map(({ value }) => value.audit_run_id)).size, 1);
  assert.equal(new Set(results.map(({ value }) => value.audit_run_row_hash)).size, 1);
  for (const { value } of results) {
    assert.equal(value.audit_result_hash, fixture.audit_result_hash);
    assert.equal(value.audit_persistence_complete, true);
    for (const field of [
  ...
  for (const field of [
    ...
    assert.equal(preflight[field], false, `${field} must remain false`);
  }
  ...
  const maxDurationMs = Math.max(...results.map(({ durationMs }) => durationMs));
  console.log(JSON.stringify({
    concurrent_callers: callerCount,
    barrier_waiters: barrierWaiters,
    bounded_sessions: boundedSessions.rows[0].count,
    inserted_results: 1,
    idempotent_results: callerCount - 1,
