-- Redact clinician names even when ordinary prose follows immediately.
-- Existing rows are intentionally not rewritten; the trigger uses this function for future writes.
...
BEGIN
  ...
  result := regexp_replace(result, '\m(Dr|Prof)\.?[[:blank:]]+(med\.?[[:blank:]]+)?[[:upper:]][[:alpha:]''-]+([[:blank:]]+[[:upper:]][[:alpha:]''-]+){0,2}', '[Name entfernt]', 'g');
