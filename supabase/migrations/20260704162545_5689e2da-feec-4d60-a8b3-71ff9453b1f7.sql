...
GRANT SELECT, INSERT, UPDATE, DELETE ON public.two_factor_pending_bindings TO authenticated;
GRANT ALL ON public.two_factor_pending_bindings TO service_role;
CREATE INDEX IF NOT EXISTS idx_two_factor_pending_bindings_user_id ON public.two_factor_pending_bindings (user_id);
CREATE INDEX IF NOT EXISTS idx_two_factor_pending_bindings_expires_at ON public.two_factor_pending_bindings (expires_at);
...
GRANT SELECT, INSERT, UPDATE, DELETE ON public.two_factor_verified_sessions TO authenticated;
GRANT ALL ON public.two_factor_verified_sessions TO service_role;
CREATE INDEX IF NOT EXISTS idx_two_factor_verified_sessions_user_id ON public.two_factor_verified_sessions (user_id);
CREATE INDEX IF NOT EXISTS idx_two_factor_verified_sessions_verified_at ON public.two_factor_verified_sessions (verified_at);
...
BEGIN
  ...
  UPDATE public.two_factor_pending_bindings SET used = true WHERE id = pending_row.id;
  DELETE FROM public.two_factor_pending_bindings WHERE expires_at <= now() OR used = true;
  DELETE FROM public.two_factor_verified_sessions WHERE verified_at <= now() - interval '7 days';
  RETURN true;
END;
$$;
...
BEGIN
  ...
  WHERE user_id = current_user_id AND session_id = current_session_id;
END;
$$;
...
CREATE POLICY "Users can view their own submissions" ON public.anamnesis_submissions FOR SELECT TO authenticated
...
CREATE POLICY "Users can insert their own submissions" ON public.anamnesis_submissions FOR INSERT TO authenticated
...
CREATE POLICY "Users can update their own submissions" ON public.anamnesis_submissions FOR UPDATE TO authenticated
USING ((select public.is_current_session_two_factor_verified()) AND auth.uid() = user_id)
WITH CHECK ((select public.is_current_session_two_factor_verified()) AND auth.uid() = user_id);
DROP POLICY IF EXISTS "Users can delete their own submissions" ON public.anamnesis_submissions;
CREATE POLICY "Users can delete their own submissions" ON public.anamnesis_submissions FOR DELETE TO authenticated
USING ((select public.is_current_session_two_factor_verified()) AND auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can view their own IAA" ON public.iaa_submissions;
CREATE POLICY "Users can view their own IAA" ON public.iaa_submissions FOR SELECT TO authenticated
USING ((select public.is_current_session_two_factor_verified()) AND auth.uid() = user_id);
DROP POLICY IF EXISTS "Users can insert their own IAA" ON public.iaa_submissions;
CREATE POLICY "Users can insert their own IAA" ON public.iaa_submissions FOR INSERT TO authenticated
WITH CHECK ((select public.is_current_session_two_factor_verified()) AND auth.uid() = user_id);
DROP POLICY IF EXISTS "Users can update their own IAA" ON public.iaa_submissions;
CREATE POLICY "Users can update their own IAA" ON public.iaa_submissions FOR UPDATE TO authenticated
USING ((select public.is_current_session_two_factor_verified()) AND auth.uid() = user_id)
WITH CHECK ((select public.is_current_session_two_factor_verified()) AND auth.uid() = user_id);

DROP POLICY IF EXISTS "Verified patients can view published resources" ON public.patient_resources;
CREATE POLICY "Verified patients can view published resources" ON public.patient_resources FOR SELECT TO authenticated
USING (is_published = true AND public.is_verified_patient(auth.uid()) AND (select public.is_current_session_two_factor_verified()));

DROP POLICY IF EXISTS "Verified patients can read patient-library objects" ON storage.objects;
CREATE POLICY "Verified patients can read patient-library objects" ON storage.objects FOR SELECT TO authenticated
USING (bucket_id = 'patient-library' AND public.is_verified_patient(auth.uid()) AND (select public.is_current_session_two_factor_verified()));

DROP POLICY IF EXISTS "Users can read own access row" ON public.patient_access;
CREATE POLICY "Users can read own access row" ON public.patient_access FOR SELECT TO authenticated
USING ((select public.is_current_session_two_factor_verified()) AND lower(email) = lower(COALESCE((auth.jwt() ->> 'email')::text, '')));
...
BEGIN
  ...
  IF user_email = '' THEN
    RETURN jsonb_build_object('has_access', false, 'anamnese_download', false, 'infothek_all', false, 'infothek_items', '[]'::jsonb, 'library_access', false);
  END IF;
  IF NOT public.is_current_session_two_factor_verified() THEN
    RETURN jsonb_build_object('has_access', false, 'anamnese_download', false, 'infothek_all', false, 'infothek_items', '[]'::jsonb, 'library_access', false);
  END IF;
  SELECT * INTO row_data FROM public.patient_access WHERE email = user_email LIMIT 1;
  IF NOT FOUND THEN
    RETURN jsonb_build_object('has_access', false, 'anamnese_download', false, 'infothek_all', false, 'infothek_items', '[]'::jsonb, 'library_access', false);
  END IF;
  RETURN jsonb_build_object('has_access', true, 'email', row_data.email, 'anamnese_download', row_data.anamnese_download, 'infothek_all', row_data.infothek_all, 'infothek_items', to_jsonb(row_data.infothek_items), 'library_access', row_data.library_access, 'updated_at', row_data.updated_at);
END;
$$;
