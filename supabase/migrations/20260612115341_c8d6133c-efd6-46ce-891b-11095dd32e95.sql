...
AS $function$
  WITH base AS MATERIALIZED (
    SELECT ts.id, row_number() OVER (ORDER BY ts.created_at DESC) AS rn
  ...
  forced AS MATERIALIZED (
    SELECT id FROM (
      (SELECT ts.id, 1 AS sort_order FROM public.therapy_sessions ts WHERE ts.pseudonym_id = _pseudonym_id AND COALESCE(ts.kind, '') NOT IN ('befund_checkpoint','quarantine_patient_mismatch','event_log','befund_auswertung') AND NULLIF(trim(COALESCE(ts.eingabe_daten->>'alter','')), '') IS NOT NULL ORDER BY ts.created_at DESC LIMIT 1)
      UNION ALL
      (SELECT ts.id, 2 AS sort_order FROM public.therapy_sessions ts WHERE ts.pseudonym_id = _pseudonym_id AND COALESCE(ts.kind, '') NOT IN ('befund_checkpoint','quarantine_patient_mismatch','event_log','befund_auswertung') AND NULLIF(trim(COALESCE(ts.eingabe_daten->>'geschlecht','')), '') IS NOT NULL ORDER BY ts.created_at DESC LIMIT 1)
      UNION ALL
      (SELECT ts.id, 3 AS sort_order FROM public.therapy_sessions ts WHERE ts.pseudonym_id = _pseudonym_id AND COALESCE(ts.kind, '') NOT IN ('befund_checkpoint','quarantine_patient_mismatch','event_log','befund_auswertung') AND NULLIF(trim(COALESCE(ts.eingabe_daten->>'symptome','')), '') IS NOT NULL ORDER BY ts.created_at DESC LIMIT 1)
      UNION ALL
      (SELECT ts.id, 4 AS sort_order FROM public.therapy_sessions ts WHERE ts.pseudonym_id = _pseudonym_id AND COALESCE(ts.kind, '') NOT IN ('befund_checkpoint','quarantine_patient_mismatch','event_log','befund_auswertung') AND NULLIF(trim(COALESCE(ts.eingabe_daten->>'medikamente','')), '') IS NOT NULL ORDER BY ts.created_at DESC LIMIT 1)
      UNION ALL
      (SELECT ts.id, 5 AS sort_order FROM public.therapy_sessions ts WHERE ts.pseudonym_id = _pseudonym_id AND COALESCE(ts.kind, '') NOT IN ('befund_checkpoint','quarantine_patient_mismatch','event_log','befund_auswertung') AND NULLIF(trim(COALESCE(ts.eingabe_daten->>'erkrankung','')), '') IS NOT NULL ORDER BY ts.created_at DESC LIMIT 1)
      UNION ALL
      (SELECT ts.id, 6 AS sort_order FROM public.therapy_sessions ts WHERE ts.pseudonym_id = _pseudonym_id AND COALESCE(ts.kind, '') NOT IN ('befund_checkpoint','quarantine_patient_mismatch','event_log','befund_auswertung') AND jsonb_typeof(ts.eingabe_daten->'manualDiagnosen') = 'array' AND jsonb_array_length(ts.eingabe_daten->'manualDiagnosen') > 0 ORDER BY ts.created_at DESC LIMIT 1)
      UNION ALL
      (SELECT ts.id, 7 AS sort_order FROM public.therapy_sessions ts WHERE ts.pseudonym_id = _pseudonym_id AND COALESCE(ts.kind, '') NOT IN ('befund_checkpoint','quarantine_patient_mismatch','event_log','befund_auswertung') AND jsonb_typeof(ts.eingabe_daten->'diagnosen') = 'array' AND jsonb_array_length(ts.eingabe_daten->'diagnosen') > 0 ORDER BY ts.created_at DESC LIMIT 1)
      UNION ALL
      (SELECT ts.id, 8 AS sort_order FROM public.therapy_sessions ts WHERE ts.pseudonym_id = _pseudonym_id AND COALESCE(ts.kind, '') NOT IN ('befund_checkpoint','quarantine_patient_mismatch','event_log','befund_auswertung') AND NULLIF(trim(COALESCE(ts.eingabe_daten->>'sonstigeUntersuchungen','')), '') IS NOT NULL ORDER BY ts.created_at DESC LIMIT 3)
    ) force_rows
    GROUP BY id
  )
  SELECT
    ...
    CASE
      WHEN COALESCE(ts.kind, '') <> 'event_log' AND (b.rn <= 12 OR f.id IS NOT NULL)
  ...
  LEFT JOIN forced f ON f.id = ts.id
  ORDER BY ts.created_at DESC;
$function$;