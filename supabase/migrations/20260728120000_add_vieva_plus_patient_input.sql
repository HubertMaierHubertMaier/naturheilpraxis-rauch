...
AS $function$
  SELECT jsonb_strip_nulls(jsonb_build_object(
    ...
    'metatronDatum', NULLIF(left(COALESCE(_input->>'metatronDatum', ''), 80), ''),
    'sonstigeUntersuchungen', NULLIF(left(COALESCE(_input->>'sonstigeUntersuchungen', ''), 2500), ''),
    'vievaPlus', NULLIF(left(COALESCE(_input->>'vievaPlus', ''), 2500), ''),
    'vievaPlusDatum', NULLIF(left(COALESCE(_input->>'vievaPlusDatum', ''), 80), ''),
...
AS $function$
  SELECT jsonb_strip_nulls(jsonb_build_object(
    ...
    'vievaPlus', NULLIF(left(COALESCE(_input->>'vievaPlus', ''), _max_chars), ''),
    'vievaPlusDatum', NULLIF(left(COALESCE(_input->>'vievaPlusDatum', ''), 80), ''),
