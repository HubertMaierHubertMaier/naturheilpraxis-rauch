...
- Dieser Teilaufruf ist ausschließlich eine kompakte, quellengetreue Extraktion. Keine zusätzlichen Pharmakologie-Abhandlungen, keine Wirkmechanismen oder Nebenwirkungen aus allgemeinem Wissen ergänzen. Dokumentierte Informationen vollständig, aber ohne Wiederholungen erfassen. Weiterführende Erklärungen gehören in einen getrennten nachgelagerten Auswertungsschritt.
...
- ALLE tatsächlich dokumentierten Medikamente, Präparate, Supplemente, Infusionen, Injektionen, OPs, Bestrahlungen, Physio-/Manual-Therapien, Heilpraktiker-Mittel in "medicationsTherapies" listen — inkl. Wirkstoff/Handelsname, Dosis falls genannt, verschreibender Arzt/Therapeut, Datum, Indikation, Status. wirkmechanismus, nebenwirkungen und grundVerordnung NUR füllen, wenn diese Informationen ausdrücklich im vorliegenden Quelltext stehen; andernfalls "". Keine Erklärung aus allgemeinem Wissen hinzufügen. Leere Medikamenten-Tabellenzeilen aus Anamnesebögen NICHT übernehmen.
...
- Extrahiere ausschließlich belegte Quellangaben. Unbekannte Wirkstoffe, Wirkmechanismen, Nebenwirkungen und Verordnungsgründe bleiben leer; keine zusätzliche Interpretation zum Füllen des Schemas erzeugen.
...
- GERÄTEINDEX IST KEINE DOSIS: Zahlen in einer Metatron-/NLS-Vorschlagsliste (z.B. "Arnica 0,120") als deviceIndex erhalten, sofern sie als Gerätewert/Resonanzindex dokumentiert sind. Niemals daraus dosis, haeufigkeit, einnahme oder dauer ableiten. Ohne ausdrückliche Einnahmeangabe sourceRole="device_suggestion", status="empfohlen" und Einnahme-/Dosisfelder leer lassen. Fehlt die eindeutige Bedeutung einer Zahl, im Beleg erhalten und als offene Frage kennzeichnen, nicht raten.
- QUELLENROLLE der Mittel zwingend trennen: sourceRole="documented_intake" nur bei dokumentierter tatsächlicher Einnahme; "lab_recommendation" für den ausdrücklich als solchen bezeichneten Therapievorschlag eines Labors (z.B. Biodiagnostik); "device_suggestion" für Vorschlags-/Testlisten eines Gerätes; "general_information" für allgemeine Mittelbeschreibungen; sonst "unclear". Labor- und Gerätevorschläge sind NICHT automatisch laufende Medikation. Vorschläge mit status="empfohlen" und Urheber in vonWem dokumentieren. Alle Angaben und Belege erhalten, keine eigene Verordnung daraus machen.
- HRV als eigene Messung erfassen: dokumentierte Werte (z.B. SDNN, RMSSD, pNN50, Frequenzparameter) samt Einheiten, Messdatum und vorhandenen Messbedingungen in findings mit findingType="hrv_measurement". Eine im Quellbericht vorhandene Deutung erhält findingType="hrv_source_interpretation". Eine zusätzliche fachliche Einordnung erhält findingType="hrv_clinical_interpretation", ist ausdrücklich als zusätzliche Einordnung zu kennzeichnen und muss sich auf belegte Messwerte beziehen. Ohne ausreichende Messbedingungen/Referenzangaben die Grenze benennen; aus HRV allein keine Diagnose ableiten. Niemals fehlende Werte oder Messdauer erfinden. Jeder Eintrag benötigt ein wörtliches Belegzitat.
- Klassisches Blutlabor und Vieva-Parameter zu Vitaminen/Mineralstoffen bleiben durch ihre Quelle und Messmethode unterscheidbar. Vieva-Angaben nicht als direkt gemessene Blutkonzentrationen bezeichnen.
- Bei labValues measurementMethod verpflichtend ausweisen: "laboratory" für dokumentierte Laboranalytik, "vieva_estimate" für Vieva-Parameter, "device_estimate" für sonstige Geräte-/Resonanzangaben, sonst "unclear". Niemals eine fehlende Messmethode erfinden. HRV gehört in die gesonderten findings, nicht in eine Blutkonzentrations-Liste.
- Quelle der Einnahme ist NICHT automatisch Quelle einer pharmakologischen Erklärung. Dieser Extraktionsschritt übernimmt nur vorhandene Aussagen; die getrennte spätere fachliche Einordnung darf nicht als Originalbefund ausgegeben werden. Keine Fachinformation, Studie, URL oder Textstelle erfinden. Kombinationen bei unbekannten Wirkstoffen niemals als unbedenklich bestätigen.
...
- Bei jedem Laborwert zusätzlich "bedeutung" und "moeglicheSymptome" auf Deutsch füllen. "bedeutung" erklärt laienverständlich, wofür der Parameter allgemein steht und was die dokumentierte Abweichungsrichtung bedeuten kann. "moeglicheSymptome" nennt nur allgemeine mögliche Beschwerden passend zur belegten Richtung und weist darauf hin, dass die Abweichung symptomlos sein kann. Diese Beschwerden niemals als beim Patienten vorhanden darstellen, wenn sie nicht als Symptom belegt sind. Bei normaler oder unklarer Bewertung keine krankhafte Bedeutung erfinden.
...
Gib ausschließlich kompaktes JSON zurück (jeder Listeneintrag ist ein Objekt mit "text" + "beleg", außer wo unten anders strukturiert):
{
  ...
  "medicationsTherapies": [{"name":"","sourceRole":"documented_intake|lab_recommendation|device_suggestion|general_information|unclear","kategorie":"konventionell|homoeopathie|pflanzenheilkunde|vitamine|mineralstoffe|spurenelemente|unklar","wirkstoff":"","dosis":"","deviceIndex":"","haeufigkeit":"","einnahme":"","dauer":"","vonWem":"","datum":"","indikation":"","wirkmechanismus":"","nebenwirkungen":"","grundVerordnung":"","status":"laufend|abgesetzt|empfohlen|unklar","polarity":"affirmed|negated|uncertain|not-stated","beleg":{"quelle":"","teil":"","seite":"","zitat":""}}],
  "labValues": [{"datum":"","parameter":"","wert":"","einheit":"","measurementMethod":"laboratory|vieva_estimate|device_estimate|unclear","referenz":"","bewertung":"normal|↑|↓|kritisch|unklar","bedeutung":"allgemeine patientenverständliche Bedeutung, keine Diagnose","moeglicheSymptome":"allgemein mögliche Beschwerden; kann symptomlos sein; keine Behauptung über den Patienten","quelle":"","beleg":{"quelle":"","teil":"","zitat":""}}],
  "findings": [{"text":"","findingType":"hrv_measurement|hrv_source_interpretation|hrv_clinical_interpretation|other","datum":"","beleg":{"quelle":"","teil":"","zitat":""}}],
...
- Bereits im Laborbericht dokumentierte Therapieempfehlungen vollständig in einer gesonderten Rubrik "Therapieempfehlung des Labors – dokumentierter Quellenvorschlag" mit Urheber, Datum und Belegen wiedergeben. Dies ist keine eigene Empfehlung und keine bestätigte Einnahme. Geräte-Vorschlagslisten und allgemeine Mittelbeschreibungen ebenfalls nicht als aktuelle Medikation darstellen.
- HRV-Messwerte und Messbedingungen, Deutung laut Quellbericht und zusätzliche fachliche Einordnung in einer eigenen HRV-Rubrik klar getrennt darstellen. Fehlende Werte/Referenzen nicht erfinden; zusätzliche Einordnung nicht dem Quellenautor zuschreiben. Klassisches Labor und Vieva-Parameter methodisch getrennt halten.
...
<body>
  ...
  ${(() => {
    ...
    return `<table><thead><tr><th>Parameter</th><th>Wert</th><th>Datum</th><th>Referenz</th><th>Richtung</th><th>Bedeutung für den Patienten</th><th>Allgemein mögliche Beschwerden</th><th>Fachliche Einordnung</th><th>Beleg</th></tr></thead><tbody>${labHighlights.map((highlight) => {
      const item = highlight.item;
      return `<tr><td><strong>${escapeHtml(item?.parameter || "—")}</strong></td><td>${escapeHtml(item?.wert || "—")} ${escapeHtml(item?.einheit || "")}</td><td>${escapeHtml(item?.datum || "(Datum unbekannt)")}</td><td>${escapeHtml(item?.referenz || "—")}</td><td>${escapeHtml(highlight.direction)}</td><td>${escapeHtml(highlight.patientMeaning)}</td><td>${escapeHtml(highlight.possibleSymptoms)}</td><td>${escapeHtml(highlight.significance)}</td><td>${beleg(item)}</td></tr>`;
    }).join("\n")}</tbody></table>`;
  })()}
