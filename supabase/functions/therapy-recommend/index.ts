...
serve(async (req) => {
  ...
  try {
    ...
    if (additionalAnamnesisText) patientInfo.push(`ZUSÄTZLICHE BEFUND- UND ANAMNESEFELDER — Quellen, Negationen und Unsicherheiten erhalten. Diagnosevorschläge sind keine gesicherten Diagnosen; frühere oder ungeklärte Präparate sind keine bestätigte aktuelle Einnahme. Therapieempfehlungen des Labors (z.B. Biodiagnostik) im Ergebnis gesondert mit Urheber, Datum und Quelle wiedergeben und vom eigenen Therapievorschlag unterscheiden; nicht automatisch als eingenommen behandeln. HRV-Messwerte, Quellendeutung und zusätzliche fachliche Einordnung getrennt verwenden; fehlende Messbedingungen oder Referenzen nicht erfinden:\n${additionalAnamnesisText}`);
